...
class PluginManager:
    ...
    def register(self, pluginDir):
        ...
        transferRequest = json.dumps({
            "fileName": 'plugin.zip',
            "size": os.stat(os.path.join(pluginDir,'dist','plugin.zip')).st_size
        })
        ...
        response = self._session.put(transferUrl, data=open(os.path.join(pluginDir,'dist','plugin.zip'), 'rb'))
...
class DirType:
    def __call__(self, value):
        ...
        if not os.path.isdir(os.path.join(value, 'dist')):
